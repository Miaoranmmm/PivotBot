...
def main():
    ...
    task2id = {'taskbot': 0, 'chitchat':1}
    id2task = {0: 'taskbot', 1: 'chitchat'}
...
#        turn_ids = examples['turn_id']
        ...
        for context, response, kb, s_kb, query, task in zip(contextes, responses, kbs, selected_kbs, querys, tasks):
            if task == 'taskbot':
                inputs.append('Tool Set : ' + truncate_string(' EOS '.join(context.split(' EOS ')[-max_his_window:]), args.max_history_length))
                state = kb.split('|')[0]
                responses_labels.append("taskbot : " + state)
                k = kb.split('|')[0]
                inputs.append('Reponse Generation : ' + truncate_string(' EOS '.join(context.split(' EOS ')[-max_his_window:]), args.max_history_length) +' <|Knowledge|> ' + k)
                responses_labels.append(response)
            elif task == 'chitchat':
                inputs.append('Tool Set : ' + truncate_string(' EOS '.join(context.split(' EOS ')[-max_his_window:]), args.max_history_length))
                responses_labels.append("chitchat : "+query)
                k = truncate_string(s_kb, args.max_knowledge_length, backward=False)
                inputs.append('Reponse Generation : ' + truncate_string(' EOS '.join(context.split(' EOS ')[-max_his_window:]), args.max_history_length) +' <|Knowledge|> ' + k)
                responses_labels.append(response)
            else:
                inputs.append('Tool Set : ' + truncate_string(' EOS '.join(context.split(' EOS ')[-max_his_window:]), args.max_history_length))
...
#        turn_ids = examples['turn_id']
        ...
        model_inputs["labels"] = labels["labels"]
        model_inputs["task"] = [task2id[t] for t in tasks]
        model_inputs["passage_ids"] = passage_ids["input_ids"]
        
        return model_inputs

    def validation_response(examples):
        passage_ids = examples['Id']
#        turn_ids = examples['turn_id']
        contextes = examples['Context']
        responses = examples['Response']
    ...
    train_dataset = raw_datasets["train"].map(
        ...
        num_proc=args.preprocessing_num_workers,
        load_from_cache_file=False,
        desc=f"Processing training dataset",
    )
    
    # eval_dataset = lm_datasets["validation"]
    eval_dataset_dst = raw_datasets["validation"].map(
        validation_state,
        batched=True,
        remove_columns=column_names,
        num_proc=args.preprocessing_num_workers,
        load_from_cache_file=False,
        desc=f"Processing validation dataset",
    )
    eval_dataset_response = raw_datasets["validation"].map(
        validation_response,
        batched=True,
        remove_columns=column_names,
        num_proc=args.preprocessing_num_workers,
    ...
    # eval_dataset_dst = eval_dataset_dst.select([0, 10, 20, 30, 40, 50])
    # eval_dataset_response = eval_dataset_response.select([0, 10, 20, 30, 40, 50])
    ...
    # Log a few random samples from the training set:
    for index in random.sample(range(len(eval_dataset_dst)), 1):
        logger.info(f"Sample {index} of the validation set: {eval_dataset_dst[index]}.")
    ...
    def postprocess_text(preds, labels):
        ...
        labels = [normalize_answer(label.strip().split('|')[-1]) for label in labels]
    ...
    eval_dst_dataloader = DataLoader(eval_dataset_dst, collate_fn=data_collator, batch_size=int(args.per_device_eval_batch_size/2))
    eval_res_dataloader = DataLoader(eval_dataset_response, collate_fn=data_collator, batch_size=int(args.per_device_eval_batch_size/2))
#    test_dataloader = DataLoader(test_dataset, collate_fn=data_collator, batch_size=args.per_device_eval_batch_size)
    ...
    model, optimizer, train_dataloader, eval_dst_dataloader, eval_res_dataloader = accelerator.prepare(
        model, optimizer, train_dataloader, eval_dst_dataloader, eval_res_dataloader
    )
    ...
    logger.info(f"  Num Eval examples = {len(eval_dataset_dst)+len(eval_dataset_response)}")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dst_dataloader):
            with torch.no_grad():
                eval_loss += model(
                            input_ids=batch["input_ids"],
                            attention_mask=batch["attention_mask"],
                ...
                # eval_size += batch['labels'].shape[0]
        ...
        # logger.info(batch['labels'].shape[0] * (step+1))
        # logger.info(eval_loss/float(step+1))
        # state_ppl = torch.exp(torch.tensor(eval_loss) / eval_size).item()
        # state_ppl = torch.exp(-torch.tensor(eval_loss) / float(step+1)).item()
        # state_bleu = metric_bleu.compute()
        # state_f1 = compute_list_f1(decoded_preds, decoded_labels)

        for step, batch in enumerate(eval_res_dataloader):
            with torch.no_grad():
                eval_loss += model(
                            input_ids=batch["input_ids"],
                            attention_mask=batch["attention_mask"],
                ...
                for i in range(len(decoded_preds)):
                    if id_list[i] not in decoded_preds_all:
                        decoded_preds_all[id_list[i].lower().replace('.json','')] = []
                    turn_result = {}
                    # turn_result['id'] = turn_ids[i]
                    turn_result['response'] = decoded_preds[i]
                    decoded_preds_all[id_list[i].lower().replace('.json','')].append(turn_result)
                    # decoded_preds_all[id_list[i]] = sorted(decoded_preds_all[id_list[i]], key=lambda d: d['id'])

        logger.info(f"  Eval Loss:  {eval_loss/(2*float(step+1))}")
        multiwoz_result = evaluator.evaluate(decoded_preds_all)
        result_inform = multiwoz_result['success']['inform'].get('total',0)
        result_success = multiwoz_result['success']['success'].get('total',0)
#        results = metric_bleu.compute()
#        logger.info(results)
        result = {}
        # result = {'state_f1': round(state_f1, 2)}
        # result = {'state_bleu': round(state_bleu['bleu']*100, 2)}
        result['eval_loss'] = eval_loss/(2*float(step+1))
        bleu = metric_bleu.compute()
        result['bleu'] = round(bleu['bleu']*100, 2)
        result['success'] = round(result_success, 2)
        result['inform'] = round(result_inform, 2)
        eval_score = result['bleu'] + 0.5 * (result['success'] + result['inform']) - result['eval_loss']
        logger.info(result)
        logger.info(eval_score)
        ...
        elif args.output_dir is not None and eval_score > best_eval_score:
            accelerator.wait_for_everyone()
            if accelerator.is_local_main_process:
